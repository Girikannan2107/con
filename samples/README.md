...
## 📂 Categorized Sample Directory Layout

```
samples/
├── tabular/                       # CSV & Structured datasets
│   ├── near_miss_reports.csv      # 18 near-miss review queue test cases
│   ├── training_corpus.csv        # 56 IOGP-labeled model training records
│   └── sample_reports.csv         # Additional structured incident data
├── documents/                     # Raw unstructured logs & files
│   ├── shift_log.txt              # Multi-block text splitting demo
│   ├── permit_observation.pdf     # Direct digital PDF text extraction
│   └── scanned_uauc_report.png    # Scanned card for PaddleOCR testing
└── multilingual/                  # Non-English datasets & language benchmarks
    ├── multilingual_report.txt    # 5 Indian languages in a single document
    └── languages/                 # Dedicated single-language incident files
        ├── hindi_report.txt
        ├── kannada_report.txt
        ├── marathi_report.txt
        ├── tamil_report.txt
...
| Category | File | Path it exercises | What to expect |
| --- | --- | --- | --- |
| **Tabular** | `tabular/near_miss_reports.csv` | **Import CSV of reports** | 18 reports; 13 flag SIF potential and all 18 reach the review queue |
| **Tabular** | `tabular/training_corpus.csv` | **Training the model** | 56 labelled reports - 33 precursors, 23 controls. See below |
| **Documents** | `documents/shift_log.txt` | **Add documents** (text) | One night-shift log; splits into 6 blocks, 5 of them analysable |
| **Documents** | `documents/permit_observation.pdf` | **Add documents** (PDF text layer) | Read without OCR by `pypdfium2`; analyses as SIF potential, risk 80.7, Work Authorisation |
| **Documents** | `documents/scanned_uauc_report.png` | **Add documents** (PaddleOCR) | A page with no text layer, so the OCR path has to run |
| **Multilingual** | `multilingual/multilingual_report.txt` | **Language / translation** | Five blocks in Hindi, Marathi, Tamil, Telugu and Kannada |
| **Multilingual** | `multilingual/languages/` | **One report per language** | Six full reports - Tamil, Hindi, Marathi, Telugu, Kannada, Urdu - each its own file, with its own README |

---

## `tabular/training_corpus.csv` - labelled data for training
...
```bash
python train_model.py samples/tabular/training_corpus.csv --encoder hashing
```
...
1. **Start with the CSV.** *Ingest and OCR → Import CSV of reports →
   `samples/tabular/near_miss_reports.csv`*. Eighteen reports analyse in a few seconds.
...
**`tabular/near_miss_reports.csv`** - the main one. Columns are `report_id`, `date`,
...
**`documents/shift_log.txt`** - blocks separated by blank lines, which is how the console
splits a document into report-sized pieces. Use it to check that one file
becomes several analysed rows.

**`documents/permit_observation.pdf`** - a permit close-out with a real text layer. It
proves the PDF path works without any OCR model present.

**`documents/scanned_uauc_report.png`** - deliberately has **no** text layer: it is a
rendered page with rotation, speckle and soft focus, so the console must fall
back to PaddleOCR. Use it to confirm an OCR install is genuinely working.

**`multilingual/multilingual_report.txt`** - the same kinds of incident written in five Indian
languages. With Ollama running, it renders each into English before
...
`multilingual/languages/` holds one complete report per language rather than blocks in one
